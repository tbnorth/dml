    ...
    def is_many_to_many(self):
        ...
        if self._is_many_to_many is None:
            self._is_many_to_many = (
                ...
                and not ("is_m2m" in self.attr and not any_to_bool(self.attr["is_m2m"]))
                and (
                    (
                        (2 <= len(self.fields) <= 3)
                        and all(self.field[f].type == "ID" for f in self.fields)
                    )
...
class Field(object):
    ...
    def __repr__(self):
        ...
        if self.foreign_key and not self.foreign_key_external:
            out.append(
                "    -> %s.%s" % (self.foreign_key.table.name, self.foreign_key.name)
            )
...
class RstOut(OutputCollector):
    ...
    def show_field(self, field):
        ...
        if field.foreign_key:
            ans.append(
                "▷%s.%s" % (field.foreign_key.table.name, field.foreign_key.name)
            )
...
class DMLOut(OutputCollector):
    ...
    def start(self, schema):
        ...
        log = E.log(
            E.log_entry("written by dml2py.py", date=time.strftime("%Y%m%d %H:%M:%S"))
        )
        self.dom.append(log)
...
class DjangoOut(OutputCollector):
    ...
    def _type_map(self, field):
        ...
        if field.primary_key:
            ...
            for k in d:
                ...
                raise Exception(
                    "Unknown Target '%s'\n\nDo you " "need `attr is_m2m False`?" % k
                )
    ...
    def start_table(self, table):
        ...
        if table.attr.get("dj_order"):
            self.emit(
                "        ordering = %s" % repr(table.attr.get("dj_order").split())
            )
        self.emit()
...
def read_dml(doc, schema):
    ...
    for table in tables:  # pass 1 - create fields
        ...
        T.comment = "\n".join([i.text or "" for i in table.xpath("./description")[:1]])
        ...
        for field in fields:
            ...
            F.comment = (
                "\n".join([i.text or "" for i in field.xpath("./description")[:1]])
            ).strip()
            F.units = (
            ...
            for f in T.fields[1:]:
                for t in T.fields[1:]:
                    ...
                    try:
                        ...
                        sys.stderr.write(
                            "Missing <attr key='is_m2m' value='false'/> " "maybe?\n"
                        )
                        sys.stderr.write("\n")
                        raise
